...
def is_identifier(e_mltag: lxml.html.HtmlElement) -> bool:
    ...
    elif e_mltag.tag == 'msub' and e_mltag[0].tag == 'mover' and e_mltag[0][0].tag == 'mi':
...
def var_html_to_str(e_mltag: lxml.html.HtmlElement):
    # mi_list は，変数記号の頭文字が名詞句の頭文字と一致しているかどうか，の判定に用いる．
    mi_list = []
    if e_mltag.tag in ['mi', 'mo', 'mn']:
        ...
        if e_mltag.tag == 'mrow':
            ...
            math_txt = r'\overset{' + mi_list[2] + '}{' + \
                r'\underset{' + mi_list[1] + \
                '}{' + mi_list[0] + '}}'
...
def extract_ml_component(
    ...
    for e_mltag_ in e_math.cssselect(mltag):
        ...
        for mi_ in mi_list_:
            mi_component = re.findall(
                '(over|under)set{(.+)}{(.+)}', mi_)
            if mi_component:
                mi_list.extend(
                    [mi_component[0][1], mi_component[0][2]])
            else:
                mi_list.append(mi_)
        # print(mi_list)
        # print(e_mltag_)
        # print(e2htmltext(e_mltag_))
        # try:
        #     print(e_mltag_.tag)
        #     print(e_mltag_[0].tag)
        #     print(e_mltag_[0][0])
        #     print(e_mltag_[0][0].text_content())
...
def extract_symbols(
    ...
    if symbol_type == 'serial':
        for e_math_ in e_math_list:
            ...
            if e_mo_text_set <= {'\u2062',
                                 '&#8290', ','} and e_mo_text_set != set():
                if e_math_[0].tag == 'mrow':
                    e_math_text_html = re.findall(
                        r'<math.*?><mrow>(.*?)</mrow></math>',
                        e2htmltext(e_math_))[0]
                else:
                    e_math_text_html = re.findall(
                        r'<math.*?>(.*?)</math>', e2htmltext(e_math_))[0]
                e_math_text_tex = e_math_.attrib.get('alttext')
                e_math_text_tex = e_math_text_tex.replace(
                    '\\displaystyle', '')
                identifier = Identifier(
                    text_tex=e_math_text_tex,
                    text_html=e_math_text_html)
                if identifier not in identifier_list:
                    identifier_list.append(identifier)
                e_math_.drop_tree()

            # TODO: 数式を数式リストに追加
            # elif '=' in e_mo_text_set:
            #     formula_list.append(
            #         Formulae(
            #             text_tex=math_text_tex,
            #             text_replaced=math_text_string,
            #             identifiers=identifiers_,
            #             replaced_string_list=replaced_string_list_)
            #             )
    ...
    for i, identifier_ in enumerate(identifier_list):
        if identifier_.id == -1:
            replaced_string_list.append(
                (identifier_.text_tex,
                    identifier_.text_html,
                    f'MATH_{i:04d}'))
