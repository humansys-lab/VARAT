...
def main():
    ...
    process_list = [
        f for f in os.listdir(DATA_FOLDER) if os.path.isdir(
            DATA_FOLDER / f)]

    process_select_expander = st.expander(
        'Select process and document.', expanded=True)

    with process_select_expander:
        process_path = DATA_FOLDER / \
            st.selectbox("Which process is used?", process_list)
    ...
    with process_select_expander:
        doc_folder_path = st.selectbox(
            ...
            paper_list,
            on_change=initialize_session_state)

    doc_tex_path = process_path / doc_folder_path / (doc_folder_path + '.tex')
    doc_html_path = process_path / \
        doc_folder_path / (doc_folder_path + '.html')
    ...
    if not os.path.isfile(doc_html_path):
        ...
        subprocess_var = subprocess.run(["latexmlc", str(doc_tex_path),
                                         "--preload=amsmath.sty",
                                        "--dest=" + str(doc_html_path)],
                                        check=True)
        st.write("Finish conversion.")

    doc_processed_path = process_path / \
        doc_folder_path / (doc_folder_path + '_preprocessed.html')

    if not os.path.isfile(doc_processed_path):
        st.write("preprocessed html file does not exist. Convert tex to html.")
        subprocess.run(
            ["python", "-m", "tools.preprocess", doc_html_path], check=True)
    ...
    for symbol_type in ['serial', 'single']:
        ...
        for replaced_string_ in replaced_string_list:
            doc_article = doc_article.replace(
                replaced_string_[1], replaced_string_[2])
    ...
    # save doc
    ##########################################################################
    doc_text_path = process_path / \
        doc_folder_path / (doc_folder_path + '_article.txt')
    if not os.path.isfile(doc_text_path):
        with open(doc_text_path, 'w') as f:
            f.write(doc_text)

    doc_article_sentence_path = process_path / \
        doc_folder_path / (doc_folder_path + '_article_sentence.txt')
    ...
    doc_article_masked_path = process_path / \
        doc_folder_path / (doc_folder_path + '_article_masked.html')
    if not os.path.isfile(doc_article_masked_path):
        with open(doc_article_masked_path, 'w') as f:
            f.write(e2htmltext(doc_article))
    ##########################################################################

    xlsx_path = process_path / doc_folder_path / \
        (doc_folder_path + '.xlsx')
    ...
    with col_left:
        with st.expander('Processed text', expanded=True):
            components.html(
                e2htmltext(doc_article),
                height=HTML_HEIGHT * 2,
                scrolling=True)
        with st.expander('Original text', expanded=False):
            components.html(
                doc_article_original,
                height=HTML_HEIGHT,
                scrolling=True)

    # TODO: これより上の処理でxlsxのinitializeをして，ここはload
    # だけにすべし（引数がxlsx_pathだけになるように設計し直す方がすっきりする）．
    # xlsxがあったら前処理不要では？
    ...
    with col_right:
        ...
        symbol_number = st.selectbox(
            ...
            key="symbol_number",
            on_change=update_form, args=(df, sentence_list,))
        ...
        if save_button:
            ...
            if sentence_extracted_list:
                if def_extracted == '':
                    ...
                    if not_one_to_one_flag or diff_len_flag:
                        ...
                        st.stop()
                    extractable_list = [str(int(def_e in def_true.split(
            ...
            n_annotated = pd.notna(
                df['definition_true']).sum()
