...
def replace_power(e_child, doc):
    if e_child.tag == "mmultiscripts":
        ...
        if is_power(e_child[-1]):
            ...
            if e_child[1].tag == 'none':
                text_after_replaced = r'<msup>' + \
                    e_child_child_list[0] + e_child_child_list[2] + r'</msup>^' + e_child_child_list[-1]
            elif e_child[2].tag == 'none':
                text_after_replaced = r'<msub>' + \
                    e_child_child_list[0] + e_child_child_list[1] + r'</msub>^' + e_child_child_list[-1]
            else:
                text_after_replaced = r'<msubsup>' + \
                    ''.join(e_child_child_list[0:3]) + r'</msubsup>^' + e_child_child_list[-1]
            doc = doc.replace(
                text_before_replaced, text_after_replaced)

    if e_child.tag == "msubsup":
        if is_power(e_child[2]):
            text_before_replaced = e2htmltext(e_child)
            text_after_replaced = r'<msub>' + e2htmltext(e_child[0]) + e2htmltext(
                e_child[1]) + r'</msub>^' + e2htmltext(e_child[2])
            doc = doc.replace(
                text_before_replaced, text_after_replaced)
    ...
    if e_child.tag == "msup":
        if is_power(e_child[1]):
            text_before_replaced = e2htmltext(e_child)
            text_after_replaced = e2htmltext(
                e_child[0]) + r'^' + e2htmltext(e_child[1])
            doc = doc.replace(
                text_before_replaced, text_after_replaced)
